...
<task type="auto">
  ...
  3. Modify extract_symbol() logic:
     a. First check for index names in the query (use INDEX_SYMBOL_MAPPING)
     b. Then check for known US tickers (existing us_tickers list)
     c. Finally, use regex but FILTER OUT matches in SYMBOL_EXCLUSIONS
     d. Return 'AAPL' as default only if nothing valid found
